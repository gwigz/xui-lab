...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    return subprocess.run([sys.executable, str(ROOT / "tools" / "check.py"), "--viewer-source", f"{fork.id}={source}"]).returncode


def scenario_paths(values: list[str]) -> list[Path]:
    return [Path(value).expanduser().resolve() for value in values] if values else sorted((ROOT / "scenarios").glob("*.json"))
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    for path in scenario_paths(args.scenarios):
        ...
        if scenario.fork != fork.id:
            raise InputError(f"{path} targets {scenario.fork}, selected fork is {fork.id}")
        result = ScenarioRunner(ROOT, fork, source, executable, artifact_root).run(scenario)
...
def cmd_interactive(args: argparse.Namespace) -> int:
    ...
    if not executable.is_file():
        raise InputError(f"runtime executable not found: {executable}")
    return subprocess.run([str(executable), "--interactive", "--resource-root", str(source.joinpath(*fork.resource_root.parts)), "--subject", args.subject]).returncode


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(prog="xui-lab")
    result.add_argument("--fork")
    result.add_argument("--viewer-source", action="append", default=[], metavar="FORK_ID=PATH")
